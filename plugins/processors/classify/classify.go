...
func (cl *Classify) initClassification() error {
	...
	if cl.DropCategories != nil {
		// A TOML array arrives here as []interface{}, whereas a config
		// built in Go may hold a []string.
		...
		case []interface{}:
			for _, elem := range v {
				s, ok := elem.(string)
				if !ok {
					return fmt.Errorf("drop_categories must be a string or array of strings, got element %v", elem)
				}
				dropCategories = append(dropCategories, s)
			}
		default:
			return fmt.Errorf("drop_categories must be a string or array of strings")
		}
		cl.DropCategories = dropCategories
